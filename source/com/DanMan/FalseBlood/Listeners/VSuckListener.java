			...
			if (Vampire.isVampire(patak.getUniqueId())) {
				...
				if (!Vampire.isVampire(pdefend.getUniqueId()) &&
					...
					if (pdefend.getFoodLevel() == 2) {
						patak.sendMessage(
							ChatColor.RED +
							"As you drain your victim their heart-beat slows.");
						patak.sendMessage(
							ChatColor.RED +
							"You know they have just enough blood to stay alive.");
					} else if (pdefend.getFoodLevel() == 0) {
						patak.sendMessage(
							ChatColor.RED +
							"You have completely endulged in your victim's blood.");
						patak.sendMessage(
							ChatColor.RED +
							"If you wish to make your victim a child vampire, right-click to give them your blood");
					}
				}
			}
		}
