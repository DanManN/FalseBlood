		...
		if (sender.hasPermission("falseblood.setage")) {
				...
				if ((sender instanceof Player)) {
					...
					try {
						...
						sender.sendMessage(ChatColor.YELLOW +
										   "Error: age didn't parse.");
						return false;
					}
				} else {
					sender.sendMessage(ChatColor.YELLOW +
									   "You can only change a player's age!");
					return true;
				}
			...
			if (player != null) {
				if (Vampire.isVampire(player.getUniqueId())) {
					...
					vamp.setAge(age);
					sender.sendMessage(ChatColor.RED + player.getName() + " is now " +
									   age + " years old!");
				} else {
					sender.sendMessage(ChatColor.YELLOW +
									   "You can only change a vampire's age!");
				}
			} else {
			...
			sender.sendMessage(ChatColor.YELLOW +
							   "You don't have the falseblood.setage permission");
		}
		return true;
	}
}
